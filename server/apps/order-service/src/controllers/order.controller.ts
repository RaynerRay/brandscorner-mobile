...
export const confirmPayment = async (
  ...
  try {
    const { clientSecret, paymentMethodData, sessionId } = req.body;

    if (!clientSecret || !paymentMethodData) {
  ...
  try {
    ...
    await prisma.notifications.create({
      data: {
        ...
        message: `Your order ${existingOrder.id.slice(0, 6)} has been updated to ${deliveryStatus}.`,
        creatorId: req.seller.id,
        receiverId: existingOrder.userId,
        redirect_link: `https://eshop.com/order/${orderId}`,
        type: "orders",
        data: existingOrder,
      },
    });
